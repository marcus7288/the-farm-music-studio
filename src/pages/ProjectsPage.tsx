...
export default function ProjectsPage() {
  ...
  const advanceStatus = (projectId: string) => {
    ...
    setProjects(prev => prev.map(p => {
      ...
      if (idx < order.length - 1) return { ...p, status: order[idx + 1] };
      return p;
    }));
    if (selected?.id === projectId) {
      setSelected(prev => {
        ...
        if (idx < order.length - 1) return { ...prev, status: order[idx + 1] };
        return prev;
      });
    }
  };
      ...
      <div style={styles.content}>
        ...
        <div style={styles.kanban}>
          {WORKFLOW_STAGES.map(stage => {
            const cards = byStatus(stage.key as ProjectStatus);
            return (
              <div key={stage.key} style={styles.kanbanCol}>
                ...
                <div style={styles.colCards}>
                  ...
                  {cards.length === 0 && <div style={styles.emptyCol}>No projects here</div>}
                </div>
              </div>
            );
          })}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  content: { maxWidth: '1200px', margin: '0 auto', padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' },
  ...
  newBtn: { background: '#3B1F0A', color: '#F5E6C8', border: 'none', borderRadius: '8px', padding: '0.6rem 1.25rem', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer' },
  ...
  kanban: { display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '0.75rem', alignItems: 'start' },
  ...
  detailPanel: { background: '#FEFAF4', width: '420px', maxWidth: '100%', padding: '2rem', overflowY: 'auto', position: 'relative', display: 'flex', flexDirection: 'column', gap: '1.25rem' },
  ...
  detailStatus: { fontSize: '0.75rem', fontWeight: 700, padding: '4px 14px', borderRadius: '99px', textTransform: 'uppercase' as const, letterSpacing: '0.04em' },
  ...
  detailTrack: { display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 0', borderBottom: '1px solid #EFE4D6', fontSize: '0.85rem' },
  trackName: { flex: 1, fontWeight: 600, color: '#3A2010' },
